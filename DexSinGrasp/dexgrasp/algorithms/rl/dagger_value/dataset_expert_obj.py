...
class ObjectTrajectoryDatasetBatch(Dataset):
    ...
    """
    def __init__(self, config, log_dir, asset_dir, trajectory_dir, object_scale_yaml, target_object_lines, expert_id, surrounding_obj_num, dtype=torch.float32, device='cuda:0'):
        ...
        self.surrounding_obj_num = surrounding_obj_num
    ...
    def _load_object_trajectory(self, nobj, ntraj):
        ...
        object_trajectory_data_path = osp.join(self.trajectory_dir, '{:04d}_seed0_expert{:01d}_obj{:01d}'.format(self.target_object_lines[nobj], self.expert_id, self.surrounding_obj_num), 'trajectory/trajectory_{:03d}.pkl'.format(ntraj))
        object_trajectory_data = load_pickle(object_trajectory_data_path)

        # print(object_trajectory_data_path)
        # print(object_trajectory_data['features'].shape)
        # print(self.config['Obs']['intervals']['object_visual'][1]-self.config['Obs']['intervals']['object_visual'][0])
        # print(object_trajectory_data['observations'].shape)
        # print(self.config['Obs']['intervals'])
        ...
        if self.vision_based:
            ...
            object_state_path = osp.join(self.trajectory_dir, '{:04d}_seed0_expert{:01d}_obj{:01d}'.format(self.target_object_lines[nobj], self.expert_id, self.surrounding_obj_num), 'pointcloud/pointcloud_{:03d}.pkl'.format(ntraj))
