...
class DAGGERVALUE:
    ...
    def __init__(self,
        ...
        if self.use_model_type == 'transformer_encoder':
            ...
            self.actor = ActorCriticTransformerEncoder(obs_shape=(self.config['Weights']['num_observation'], ), states_shape=None, actions_shape=(self.config['Weights']['num_action'], ), initial_std=0.8, model_cfg=self.config)
            self.actor.to(self.device)
            # print("ActorCriticTransformerEncoder architecture:", self.actor)  # Print decoder model architecture
            ...
            for it in range(self.vec_env.task.test_iteration):
                ...
                for _ in range(self.vec_env.task.max_episode_length):
                    ...
                    with torch.no_grad():
                        ...
                        # Save observations to txt file
                        # observations_file = os.path.join('observations.txt')
                        # with open(observations_file, 'a') as f:
                        #     # Convert observations tensor to numpy and save each observation vector on a new line
                        ...
                        # Save actions to txt file
                        # actions_file = os.path.join('actions.txt')
                        # with open(actions_file, 'a') as f:
                        #     # Convert actions tensor to numpy and save each action vector on a new line
                    ...
                    if _ == self.vec_env.task.max_episode_length-2:
                        ...
                        final_success_rate_temp=self.vec_env.task.final_successes.sum()/self.vec_env.num_envs
                        print("Success rate: {:.2%}, {}/{}".format(success_rate, self.vec_env.task.successes.sum(), self.vec_env.num_envs))
                        print("Final success rate: {:.2%}, {}/{}".format(final_success_rate_temp, self.vec_env.task.final_successes.sum(), self.vec_env.num_envs))
                        if obs_action_trajectory is not None: 
