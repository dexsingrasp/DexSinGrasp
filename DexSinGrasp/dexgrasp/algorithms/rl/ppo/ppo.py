    ...
    def run(self, num_learning_iterations, log_interval=1):
        ...
        if self.is_testing:
            ...
            for it in range(self.vec_env.task.test_iteration):
                ...
                final_average_step.append(self.vec_env.task.current_avg_steps)
                np.savetxt(os.path.join(self.vec_env.task.render_folder, 'final_average_step.txt'), np.asarray(final_average_step))
        ...
        # train PPO within env  
            ...
            for it in range(self.current_learning_iteration, num_learning_iterations):
                ...
                if it % (max(1, 100))  == 0:
                    ...
                    print_logs.append(f"Current success rate: {self.vec_env.task.current_success_rate}, Cumulative successes: {self.vec_env.task.cumulative_successes}")
                    print_logs.append(f"Current average step: {self.vec_env.task.current_avg_steps}, Cumulative average step: {self.vec_env.task.avg_succ_steps}")
                    print_logs.append(f"Current reward: {self.vec_env.task.reward_value}")
                    print(print_logs[-4:])
