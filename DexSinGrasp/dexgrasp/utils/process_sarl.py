...
def process_dagger_value(args, env, cfg_train, logdir):
    ...
    if args.model_dir != "":
        base_path = '/'.join(logdir.split('/')[:-3]) 
        exp_path = f'{args.model_dir}/model_best.pt'
        chkpt_path = os.path.join(base_path, exp_path)
    else:
        base_path = '/'.join(logdir.split('/')[:-3]) 
        exp_path = f'distill_student_model/model_best.pt'
        chkpt_path = os.path.join(base_path, exp_path)
...
# init ppo model, load model for ActorCritic, test or train model
def process_ppo(args, env, cfg_train, logdir):
    ...
    logdir = logdir + "_seed{}".format(env.task.cfg["seed"])+ "_expert{}".format(env.task.expert_id)+ "_obj{}".format(env.task.surrounding_obj_num)
    ...
    if is_testing and args.model_dir != "":
        print("Loading ppo model for testing from {}".format(chkpt_path))
        ppo.test(chkpt_path)
    elif args.model_dir != "":
        print("Loading ppo model for training from {}".format(chkpt_path))
        ppo.load(chkpt_path)
